...
public:
    ...
    AdaptiveSequence<T> operator+(const AdaptiveSequence<T>& other) const {
        int newSize = std::min(size, other.size);
        T* result = new T[newSize];
        for (int i = 0; i < newSize; i++) {
            result[i] = data[head + i] + other.data[other.head + i];
        }
        AdaptiveSequence<T> newSeq(result, newSize);
        delete[] result;
        return newSeq;
    }

    AdaptiveSequence<T> operator*(const AdaptiveSequence<T>& other) const {
        int newSize = std::min(this->size, other.size);
        T* result = new T[newSize];
        for (int i = 0; i < newSize; i++) {
            result[i] = data[head + i] * other.data[other.head + i];
        }
